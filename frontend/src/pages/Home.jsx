...
const getPropertyTitle = (property) => property?.title || "Untitled Property";

const getPropertyImage = (property, fallback) => {
  const imageFromArray = Array.isArray(property?.images) ? property.images[0] : "";
  return property?.image || imageFromArray || fallback;
};
...
function BigPropertyCard({ property }) {
  ...
  return (
    ...
    >
      <img
        src={getPropertyImage(property, "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=800&q=80")}
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: 20 }}>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 28, fontWeight: 700, color: "#fff", marginBottom: 2 }}>{getPropertyTitle(property)}</p>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: "rgba(255,255,255,0.6)", marginBottom: 8 }}>{getLocationLabel(property.location)}</p>
  ...
  return (
    ...
    >
      <Link to={`/property/${property._id}`} style={{ display: "block", overflow: "hidden", height: 220 }}>
        <img src={getPropertyImage(property, "https://images.unsplash.com/photo-1560518883-ce09059eeffa?q=80&w=1200&auto=format&fit=crop")} alt={getPropertyTitle(property)} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transition: "transform 0.5s", transform: hovered ? "scale(1.05)" : "scale(1)" }} />
      </Link>
      <div style={{ padding: "14px 16px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        <div style={{ flex: 1 }}>
          <Link to={`/property/${property._id}`} style={{ textDecoration: "none" }}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 18, fontWeight: 600, color: "#0f172a", marginBottom: 4 }}>{getPropertyTitle(property)}</p>
          </Link>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#94a3b8", marginBottom: 6 }}>{getLocationLabel(property.location)}</p>
    ...
    <>
      ...
      <div style={{ background: "linear-gradient(180deg,#ffffff 0%,#f8fafc 52%,#ffffff 100%)", minHeight: "100vh" }}>
        ...
        <section style={{ marginLeft: 30, marginRight: 30, position: "relative", height: 820, borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", zIndex: 2, height: "100%", maxWidth: 1600, margin: "0 auto", padding: "0 60px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 40 }}>
